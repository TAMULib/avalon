...
context('Actions', () => {
  ...
  it('.sign_in() - click on a DOM element', () => {
	...
	cy.get('form').within(() => {
		cy.get('#auth_key').type('sumith@example.com').should('have.value', 'sumith@example.com') // Only yield inputs within form
		cy.get('#password').type('sumith') // Only yield textareas within form
		})
	cy.get('button[type=submit]').last().click()

	cy.get('#playlists_nav').first().click()

  })

  it('.public_user() - click on a DOM element', () => {
	cy.get('a[href*="catalog"] ').first().click()
	
	//cy.get(' a[href*="/media_objects/"] ').first().click()
	cy.get('a[href*="media_objects').then((media_objects) => {

		//const media = media_objects[2]
		//media.click()
		//var myWindow = window.open('','','width=200,height=100')
		//myWindow.document.write(media_objects)
